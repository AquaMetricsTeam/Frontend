...
function Drawer({
  ...
  const activeDirection = (direction ?? swipeDirection ?? "down") as NonNullable<
    DrawerPrimitive.Root.Props["swipeDirection"]
  >
  const hasSnapPoints = snapPoints != null && snapPoints.length > 0
  const contextValue = React.useMemo(
    () => ({ hasSnapPoints, modal, showSwipeHandle, swipeDirection: activeDirection }),
    [hasSnapPoints, modal, showSwipeHandle, activeDirection]
  )
